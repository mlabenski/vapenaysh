<template>
	<div class="p-6 bg-white rounded shadow-md">
		...
		<div class="mb-4 flex">
			...
			<v-select v-model="localProduct.brand_name" :options="availableBrands" class="flex-grow"
		...
		<div class="mb-4">
			<label for="category" class="block text-sm font-bold mb-2">Flavor:</label>
			<textarea v-model="localProduct.flavor" rows="4" placeholder="Enter product flavor" class="border p-2 w-full"></textarea>
		</div>

		<div class="mb-4">
			<label for="category" class="block text-sm font-bold mb-2">Category:</label>
			<select v-model="localProduct.categories" @change="updateNicotineOptions" class="border p-2 w-full">
		...
		<div class="mb-4">
			...
			<select v-model="localProduct.bottle_size" class="border p-2 w-full">
...
.add-brand-option {
	...
	color: blue;
	/* Base clickable color */
	transition: color 0.3s ease, background-color 0.3s ease, box-shadow 0.3s ease;
	/* Smooth transitions */
	padding: 4px 8px;
	/* Some spacing to make it more button-like */
	border-radius: 4px;
	/* Rounded corners */
}
