<template>
  <div class="bg-gray-100 h-[calc(100vh-90px)] overflow-auto relative">
    ...
    <div class="w-full float-right p-4 bg-white rounded">
      ...
      <multiselect
        v-else-if="column.field === 'stores' && storeEdit"
        style="color: black"
        v-model="row.stores"
        :options="allStores"
        :multiple="true"
      ...
      v-else-if="column.field === 'categories' && categoryEdit"
        ...
        @input="onPropertyEdit(row, 'categories')"
        >
        </multiselect>
        <input
          v-else-if="isRowEditable(row) && column.field !== 'categories'"
          v-model="row[column.field]"
          @input="onPropertyEdit(row, column.field)"
          type="text"
          class="table-input"
        />
      </template>
