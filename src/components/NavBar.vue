<template>
     <div v-if="!isMobile || showNavOnMobile" class="p-5 pl-2 resize-nav-bar">
      ...
      <div v-if="!isLoading">
        <template v-for="item in navbarItems">
          <div :key="item.name" @click="toggleItem(item.name)" class="mb-2.5 py-2.5 cursor-pointer flex w-full hover:bg-gray-100 hover:border-l-4 hover:border-gray-300">
          ...
          <div v-if="item.name === 'store' && expandedItems[item.name]" class="mt-5">
            <template v-for="subItem in myData">
              <div :key="subItem.storeID" @click="emitNavigation('store', subItem.storeID)" class="ml-7.5 mt-2.5 py-1.25 cursor-pointer hover:bg-gray-150 hover:border-l-4 hover:border-gray-700">
                  {{ subItem.name_dba }}
              </div>
            </template>
          </div>
            ...
            <div v-else-if="expandedItems[item.name] && item.subItems.length" class="mt-5">
              <template v-for="subItem in item.subItems">
                <div :key="subItem" @click="emitNavigation(item.name, subItem)" class="ml-7.5 mt-2.5 py-1.25 cursor-pointer hover:bg-gray-150 hover:border-l-4 hover:border-gray-700">
                    {{ subItem }}
                </div>
              </template>
            </div>
    ...
    <button class="w-1/6 p-2 rounded relative">
        <!-- Edit Icon (Default) -->
        <font-awesome-icon :icon="['fas', 'edit']" class="absolute top-0 left-0 w-full h-full text-gray-700 hover:hidden" />
        
        <!-- Hover Icon for Edit (Shown on hover) -->
        <font-awesome-icon :icon="['fas', 'edit']" bounce class="absolute top-0 left-0 w-full h-full text-gray-700 hidden hover:block" />
    </button>

    <button class="w-1/6 p-2 m-2 rounded relative">
        <!-- Trash Icon (Default) -->
        <font-awesome-icon :icon="['fas', 'trash']" class="absolute top-0 left-0 w-full h-full text-gray-700 hover:hidden" />
        
        <!-- Hover Icon for Trash (Shown on hover) -->
        <font-awesome-icon :icon="['fas', 'trash']" bounce class="absolute top-0 left-0 w-full h-full text-gray-700 hidden hover:block" />
    </button>
</div>
    </div>
  </div>
